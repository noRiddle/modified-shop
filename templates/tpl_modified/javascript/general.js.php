<?php
/* -----------------------------------------------------------------------------------------
   $Id: general.js.php 143 2012-12-03 09:22:31Z Markus $
...
echo '<script src="http' . (($request_type=='SSL')?'s':'') . '://ajax.googleapis.com/ajax/libs/jquery/1/jquery.min.js" type="text/javascript"></script>';
//EOF - DokuMan - 2011-05-12 - load jQuery default library jquery.js from faster Google CDN ?>
<script type="text/javascript">
//<![CDATA[
  !window.jQuery && document.write('<script src="<?php echo DIR_WS_BASE; ?>templates/<?php echo CURRENT_TEMPLATE; ?>/javascript/jquery.js" type="text/javascript"><\/script>');
//]]>
</script>
...
$(document).ready(function() {
  ...
  // Amount of wait time before shopping-cart slides back up
  var SlideDelay = 1500;
  // The speed of the slide down
  var SlideSpeed = 500;
  
  $("#cart_short").hover(
    function () {
      $(this).addClass('imHovered'); 
      $("#cart_long").slideDown("slow");
    },
    function () {
      ...
      this.timer = setTimeout(function(){
        if (!$('#cart_long').hasClass('imHovered') && !$('#btn').hasClass('imHovered')) {
          $('#cart_long').slideUp(SlideSpeed);
        }
      }, SlideDelay);
  });
  ...
  $("#cart_long").hover(
    ...
    function(){ 
      ...
      this.timer = setTimeout(function(){
        if (!$('#cart_short').hasClass('imHovered') && !$('#cart_long').hasClass('imHovered')) {
          $('#cart_long').slideUp(SlideSpeed);
        }
      }, SlideDelay);
    }
...
<?php
  ...
  if (strstr($PHP_SELF, FILENAME_PRODUCT_INFO )) {
    ...
    echo '<script src="http' . (($request_type=='SSL')?'s':'') . '://ajax.googleapis.com/ajax/libs/jqueryui/1.8.16/jquery-ui.min.js" type="text/javascript"></script>';
...
<script type="text/javascript">
	$(document).ready(function(){
		$("#admin li.togglemenu").mouseover(function() {
			$(this).children("ul").css("display","block");
		}).mouseout(function() {
			$(this).children("ul").css("display","none");
		});

    $(".togglebox").css("cursor", "pointer").prepend("<span class='togglestate'></span>");
    $(".togglebox").click(function() {
      if ($(this).parent(".togglebox").hasClass("togglestate_open")) {
        $(this).parent(".togglebox").removeClass("togglestate_open").addClass("togglestate_close");
      } else {
        $(this).parent(".togglebox").removeClass("togglestate_close").addClass("togglestate_open");
      }
      return false;
    });
