<?php
  /* -----------------------------------------------------------------------------------------
   $Id: best_sellers.php 2 2012-11-21 15:30:32Z GTB $
  ...
  if (!$box_smarty->is_cached(CURRENT_TEMPLATE.'/boxes/box_best_sellers_footer.html', $cache_id) || !$cache) {
    ...
    */
    if (count($box_content) > 0) {
      $box_best_sellers_footer = '';
      // set cache ID
      if (!$cache) {
        if ($box_content!='') {
          $box_best_sellers_footer = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_best_sellers_footer.html');
        }
      } else {
        $box_best_sellers_footer = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_best_sellers_footer.html', $cache_id);
      }
      $smarty->assign('box_BESTSELLERS_FOOTER', $box_best_sellers_footer);
    }
  }
  //EOF - DokuMan - 2010-07-12 - fix Smarty cache error on unlink
?>