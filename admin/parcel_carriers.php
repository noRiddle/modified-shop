  ...
  if (xtc_not_null($action)) {
    switch ($action) {
      ...
      case 'save':
        ...
        xtc_redirect(xtc_href_link(FILENAME_PARCEL_CARRIERS, 'page=' . $_GET['page'] . '&carrierID=' . $carrier_id));
        break;

      case 'deleteconfirm':
        $carrier_id = xtc_db_prepare_input($_GET['carrierID']);
        xtc_db_query("delete from " . TABLE_CARRIERS . " where carrier_id = '" . (int)$carrier_id . "'");
        xtc_redirect(xtc_href_link(FILENAME_PARCEL_CARRIERS, 'page=' . $_GET['page']));
        break;
    }
  }
                  ...
                  <tr>
                    <td valign="top">
                      <table border="0" width="100%" cellspacing="0" cellpadding="2">
                        ...
                        $carriers_query_raw = "select
                                                ...
                                                from " . TABLE_CARRIERS . " order by carrier_sort_order";
                        $carriers_split = new splitPageResults($_GET['page'], MAX_DISPLAY_SEARCH_RESULTS, $carriers_query_raw, $carriers_query_numrows);
                        ...
                        while ($carriers = xtc_db_fetch_array($carriers_query)) {
                          ...
                          if (isset($carriersInfo) && is_object($carriersInfo) && ($carriers['carrier_id'] == $carriersInfo->carrier_id) ) {
                            echo '              <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'pointer\'" onclick="document.location.href=\'' . xtc_href_link(FILENAME_PARCEL_CARRIERS, 'page=' . $_GET['page'] . '&carrierID=' . $carriersInfo->carrier_id . '&action=edit') . '\'">' . "\n";
                          } else {
                            echo'              <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'pointer\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . xtc_href_link(FILENAME_PARCEL_CARRIERS, 'page=' . $_GET['page'] . '&carrierID=' . $carriers['carrier_id']) . '\'">' . "\n";
                          }
                            ?>
                        ...
                        <tr>
                          <td colspan="4">
                            <table border="0" width="100%" cellspacing="0" cellpadding="2">
                              <tr>
                                <td class="smallText" valign="top"><?php echo $carriers_split->display_count($carriers_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_CARRIERS); ?></td>
                                <td class="smallText" align="right"><?php echo $carriers_split->display_links($carriers_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $_GET['page']); ?></td>
                              </tr>
                              <?php
                              if (empty($action)) {
                                ?>
                                <tr>
                                  <td colspan="2" align="right"><?php echo '<a class="button" onclick="this.blur();" href="' . xtc_href_link(FILENAME_PARCEL_CARRIERS, 'page=' . $_GET['page'] . '&action=new') . '">' . BUTTON_NEW_CARRIER . '</a>'; ?></td>
                                </tr>
                                <?php
                              }
                              ?>
                    ...
                    switch ($action) {
                      case 'new':
                        ...
                        $contents = array('form' => xtc_draw_form('carrier', FILENAME_PARCEL_CARRIERS, 'page=' . $_GET['page'] . '&action=insert'));
                        $contents[] = array('text' => TEXT_INFO_INSERT_INTRO);
                        $contents[] = array('text' => '<br />' . TEXT_INFO_CARRIER_NAME . '<br />' . xtc_draw_input_field('carrier_name'));
                        $contents[] = array('text' => '<br />' . TEXT_INFO_CARRIER_TRACKING_LINK . '<br />' . xtc_draw_input_field('carrier_tracking_link'));
                        ...
                        $contents[] = array('align' => 'center', 'text' => '<br /><input type="submit" class="button" onclick="this.blur();" value="' . BUTTON_INSERT . '"/>&nbsp;<a class="button" onclick="this.blur();" href="' . xtc_href_link(FILENAME_PARCEL_CARRIERS, 'page=' . $_GET['page']) . '">' . BUTTON_CANCEL . '</a>');
                        break;
                      case 'edit':
                        $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_EDIT_CARRIER . '</b>');
                        $contents = array('form' => xtc_draw_form('carrier', FILENAME_PARCEL_CARRIERS, 'page=' . $_GET['page'] . '&carrierID=' . $carriersInfo->carrier_id . '&action=save'));
                        $contents[] = array('text' => TEXT_INFO_EDIT_INTRO);
                        $contents[] = array('text' => '<br />' . TEXT_INFO_CARRIER_NAME . '<br />' . xtc_draw_input_field('carrier_name', $carriersInfo->carrier_name));
                        $contents[] = array('text' => '<br />' . TEXT_INFO_CARRIER_TRACKING_LINK . '<br />' . xtc_draw_input_field('carrier_tracking_link', $carriersInfo->carrier_tracking_link));
                        $contents[] = array('text' => '<br />' . TEXT_INFO_CARRIER_SORT_ORDER . '<br />' . xtc_draw_input_field('carrier_sort_order', $carriersInfo->carrier_sort_order));
                        $contents[] = array('align' => 'center', 'text' => '<br /><input type="submit" class="button" onclick="this.blur();" value="' . BUTTON_UPDATE . '"/>&nbsp;<a class="button" onclick="this.blur();" href="' . xtc_href_link(FILENAME_PARCEL_CARRIERS, 'page=' . $_GET['page'] . '&carrierID=' . $carriersInfo->carrier_id) . '">' . BUTTON_CANCEL . '</a>');
                        break;
                      case 'delete':
                        $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_DELETE_CARRIER . '</b>');
                        $contents = array('form' => xtc_draw_form('carrier', FILENAME_PARCEL_CARRIERS, 'page=' . $_GET['page'] . '&carrierID=' . $carriersInfo->carrier_id . '&action=deleteconfirm'));
                        $contents[] = array('text' => TEXT_INFO_DELETE_INTRO);
                        $contents[] = array('text' => '<br /><b>' . $carriersInfo->carrier_name . '</b>');
                        $contents[] = array('align' => 'center', 'text' => '<br /><input type="submit" class="button" onclick="this.blur();" value="' . BUTTON_DELETE . '"/>&nbsp;<a class="button" onclick="this.blur();" href="' . xtc_href_link(FILENAME_PARCEL_CARRIERS, 'page=' . $_GET['page'] . '&carrierID=' . $carriersInfo->carrier_id) . '">' . BUTTON_CANCEL . '</a>');
                        break;
                      default:
                        if (isset($carriersInfo) && is_object($carriersInfo)) {
                          $heading[] = array('text' => '<b>' . $carriersInfo->carrier_name . '</b>');
                          $contents[] = array('align' => 'center', 'text' => '<a class="button" onclick="this.blur();" href="' . xtc_href_link(FILENAME_PARCEL_CARRIERS, 'page=' . $_GET['page'] . '&carrierID=' . $carriersInfo->carrier_id . '&action=edit') . '">' . BUTTON_EDIT . '</a> <a class="button" onclick="this.blur();" href="' . xtc_href_link(FILENAME_PARCEL_CARRIERS, 'page=' . $_GET['page'] . '&carrierID=' . $carriersInfo->carrier_id . '&action=delete') . '">' . BUTTON_DELETE . '</a>');
