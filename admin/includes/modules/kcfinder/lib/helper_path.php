...
/** Get the absolute URL path of the given one. Returns FALSE if the URL
  ...
  * @return string */
    ...
    static function rel2abs_url($path) {
        ...
        if (!isset($_SESSION['KCFINDER']['DOCUMENT_ROOT']) || ($dir === false))
            return false;

        $dir = self::normalize($dir);
        $doc_root = self::normalize(realpath($_SESSION['KCFINDER']['DOCUMENT_ROOT']));
...
/** Resolve full filesystem path of given URL. Returns FALSE if the URL
  ...
  * @return string */
    ...
    static function url2fullPath($url) {
        ...
        if (isset($_SESSION['KCFINDER']['DOCUMENT_ROOT'])) {
            return self::normalize(realpath($_SESSION['KCFINDER']['DOCUMENT_ROOT']) . "/$url");
