...
/* -----------------------------------------------------------------------------------------
  ...
  {
    $total_amount = $xtPrice->xtcGetPrice($product->data['products_id'], false, 1, $product->data['products_tax_class_id'], $product->data['products_price']); 
        ...
        )
    {
      ...
      if ($paypal_installment->get_config('MODULE_PAYMENT_'.strtoupper($paypal_installment->code).'_UPSTREAM_PRODUCT') == '1') {
      ...
      $pp_smarty->assign('notice', TEXT_PAYPALINSTALLMENT_NOTICE_PRODUCT);
