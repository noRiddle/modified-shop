	...
	*/
	...
	class ShopgateInstallHelper {
		...
		private function getUid() {
			...
			$keyQuery = 'SELECT c.configuration_value AS val FROM ' . TABLE_CONFIGURATION . ' AS c WHERE c.configuration_key = "' . self::SHOPGATE_DATABASE_CONFIG_KEY . '" LIMIT 1;';
			...
			$updateKeyQuery = 'UPDATE ' . TABLE_CONFIGURATION . ' AS c  SET c.configuration_value ="' . $saltedHash . '" WHERE c.configuration_key = "' . self::SHOPGATE_DATABASE_CONFIG_KEY . '";';
		...
		private function getStoreHolderInformation() {
			...
			$keyQuery = 'SELECT configuration_key,configuration_value FROM ' . TABLE_CONFIGURATION . ' as c
						 WHERE c.configuration_key = "' . self::SHOPGATE_DEFAULT_EMAIL_KEY . '"
						 OR c.configuration_key    = "' . self::SHOPGATE_DEFAULT_CONTACT_NAME_KEY . '" 
						 OR c.configuration_key    = "' . self::SHOPGATE_DEFAULT_STORE_NAME_KEY . '"
						 OR c.configuration_key    = "' . self::SHOPGATE_DEFAULT_STORE_NAME_ADDRESS_KEY . '";';
		...
		private function getOrderAmount($beginDate = NULL) {
			...
			$query  = "SELECT count(*) as cnt FROM " . TABLE_ORDERS . " WHERE date_purchased BETWEEN '{$beginDate}' AND now()";
			$result = xtc_db_query($query);
			$row    = xtc_db_fetch_array($result);
			return $row[ 'cnt' ];
		}
		...
		public function getAcs() {
			...
			$query = "SELECT (SUM(op.products_price) DIV (SELECT COUNT(*) FROM orders_products)) as acs FROM orders_products as op";
			$result = xtc_db_query($query);
			$row    = xtc_db_fetch_array($result);
			if (!empty($row)) {
		...
		private function getDefaultCurrency() {
			$query  = 'SELECT configuration_value AS currency FROM ' . TABLE_CONFIGURATION . ' AS c
				   where c.configuration_key = "' . self::SHOPGATE_DEFAULT_CURRENCY_KEY . '"';
			$result = xtc_db_query($query);
			$row    = xtc_db_fetch_array($result);
			return $row[ 'currency' ];
		}
