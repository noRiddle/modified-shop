 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: AmazonCheckinSubmit.php 4935 2014-12-01 15:31:32Z tim.neumann $
 *
...
class AmazonCheckinSubmitOld extends CheckinSubmit {
	private $checkinDetails = array();
	...
	protected function getVariations($pID, $product, &$data) {
		...
		/* Sicherstellen daß die Varianten nur 1 Dimension haben
		   (Amazon kann nix anderes)
		 */
			if (    (MagnaDB::gi()->fetchOne('
					SELECT COUNT(*) FROM products_properties_combis
					  WHERE products_id = \''.$pID.'\''))
		         == (MagnaDB::gi()->fetchOne('
					SELECT COUNT(*) FROM products_properties_index
					  WHERE products_id = \''.$pID.'\'
					 AND language_id = \''.getDBConfigValue(
			                $this->_magnasession['currentPlatform'].'.lang',
			                $this->_magnasession['mpID'],
			                $_SESSION['languages_id']
			         ).'\''))
			...
			$variationTheme = MagnaDB::gi()->fetchArray(eecho('
				SELECT ppi.properties_name AS VariationTitle,
					   ppi.values_name AS VariationValue,
					   ppc.products_properties_combis_id AS aID,
					   ABS(ppi.values_price) AS aPrice,
					   IF (ppi.values_price < 0, \'-\', \'+\') AS aPricePrefix,
					   ppc.combi_quantity AS Quantity,
					   ppc.combi_ean AS EAN
				  FROM products_properties_index ppi, products_properties_combis ppc
				 WHERE ppi.products_id = \''.$pID.'\'
				   AND ppc.products_id = \''.$pID.'\'
		...
		} else if (defined('MAGNA_FIELD_ATTRIBUTES_EAN') 
			&& MagnaDB::gi()->columnExistsInTable('attributes_stock', TABLE_PRODUCTS_ATTRIBUTES)
		) {
			$variationTheme = MagnaDB::gi()->fetchArray(eecho('
			    SELECT po.products_options_name AS VariationTitle,
